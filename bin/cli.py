...
from ser.transforms import transforms, normalize, flip as flip_transform
...
def infer(
    ...
    flip: bool = typer.Option(False, "-f", "--flip", help="Flip the image")
):
...
def _select_test_image(label, flip=False):
...
def _configure_transforms(flip):
    if flip:
       ts = [normalize, flip_transform]
    else: ts = [normalize]
    return ts
